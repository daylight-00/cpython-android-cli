...
## 20. RB-7 non-Termux Android runtime support-scope disposition

The accepted runtime evidence is confined to the Termux app-private owner/runtime lane on the exact frozen AArch64 family. The owner declines creating or acquiring a separate non-Termux Android app-private or shell context solely for this gate. Termux execution cannot prove independence from Termux native providers, and x86 host or emulator execution cannot substitute for the frozen AArch64 Android artifact.

RB-7 may therefore close only through an explicit owner support-scope exclusion. The transition must set non-Termux Android runtime qualification and support to false, preserve the accepted Termux runtime evidence, keep minimum supported Android API undeclared, and leave frozen artifact and data bytes unchanged. The owner runner must verify that it is executing inside the Termux context; a genuinely non-Termux or unidentified context fails closed because separate exact-context qualification would then be required. After RB-7 closes, the only remaining release blocker is RB-1 explicit owner approval of the exact canonical successor r3 release and bound notice set. Selectability and publication remain false until that approval and a separate final release-candidate integration transition.
## 21. RB-5/RB-7 runtime support-policy correction

The owner supersedes only the support-language portions of the earlier RB-5 and RB-7 scope dispositions. The exact artifact family was built with Android API 24 as its ABI floor and reports `android-24-arm64_v8a`; API 24 is therefore declared as the minimum supported Android API without claiming exact API 24 device validation. `api24_runtime_qualified` and `api24_runtime_device_validated` remain false, while `api24_runtime_supported` becomes true and scope exclusion is removed.
