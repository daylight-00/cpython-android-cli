...
def verify(root:Path=ROOT)->dict[str,Any]:
 ...
 ids=authority.get('file_identities',{});second_replacements=second.get('replacements',{});latest_replacements=latest.get('replacements',{})
 id_checks={rel:(root/rel).is_file() and sha(root/rel)==latest_replacements.get(rel,second_replacements.get(rel,{})).get('replacement_sha256',expected) for rel,expected in ids.items()}
