...
def verify(root:Path=ROOT)->dict[str,Any]:
 ...
 scope_progression=state.get('state_revision')==56 and state.get('active_work_package')==rb6_contract and state.get('claim_boundaries',{}).get('rb5_closed') is True and state.get('claim_boundaries',{}).get('api24_runtime_supported') is False and state.get('claim_boundaries',{}).get('api24_runtime_scope_excluded') is True and task.get('deliverable',{}).get('current_bounded_transition')=='rb6-real-16k-runtime-support-disposition' and {scope_authority,scope_temporal,rb6_contract}.issubset(task_reads) and all((root/p).is_file() for p in (scope_authority,scope_temporal,rb6_contract)) and task_auth.get(scope_authority)==sha(root/scope_authority) and task_auth.get(scope_temporal)==sha(root/scope_temporal)
 ...
 checks={
  ...
  'state':(state.get('state_revision',0)>=54 and state.get('claim_boundaries',{}).get('rb4_closed') is True and state.get('active_work_package')=='experiments/epoch3-upstream-thin-release-blockers/rb5-api24-runtime-contract.json' and 'RB-4-release-security-update-and-revocation-operations' not in state.get('blockers',[]) and (state.get('state_revision')==54 or (state.get('claim_boundaries',{}).get('api24_runtime_started') is True and state.get('claim_boundaries',{}).get('api24_runtime_candidate') is False and state.get('claim_boundaries',{}).get('api24_runtime_accepted') is False))) or scope_progression or rb6_scope_progression or rb7_scope_progression or policy_correction_progression,
  'task':(task.get('state_revision',0)>=54 and task.get('task',{}).get('work_class')=='T' and task.get('deliverable',{}).get('current_bounded_transition')=='rb5-api24-runtime-owner-qualification' and (task.get('state_revision')==54 or any(x.get('path')=='experiments/epoch3-upstream-thin-release-blockers/rb5-api24-runtime-execution-contract.json' for x in task.get('required_reads',[])))) or scope_progression or rb6_scope_progression or rb7_scope_progression or policy_correction_progression,
