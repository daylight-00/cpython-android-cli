...
def verify(root: Path = ROOT) -> dict[str, Any]:
    ...
    checks = {
        ...
        'state': (state.get('state_revision') == 56 and state.get('active_work_package') == 'experiments/epoch3-upstream-thin-release-blockers/rb6-real-16k-runtime-support-disposition-contract.json' and 'RB-5-api24-runtime-qualification' not in state.get('blockers', []) and sb.get('rb5_closed') is True and sb.get('api24_runtime_supported') is False and sb.get('api24_runtime_scope_excluded') is True and sb.get('minimum_supported_android_api_declared') is False and sb.get('selectable') is False) or rb6_scope_progression or rb7_scope_progression,
        'task': (task.get('state_revision') == 56 and task.get('deliverable', {}).get('current_bounded_transition') == 'rb6-real-16k-runtime-support-disposition' and task.get('claim_boundary', {}).get('rb5_closed_claim') is True and task.get('claim_boundary', {}).get('api24_runtime_scope_excluded_claim') is True) or rb6_scope_progression or rb7_scope_progression,
        ...
        'register_rb6': (rb6r.get('status') == 'open-owner-support-disposition' and rb6r.get('evidence', {}).get('contract_sha256') == EXPECTED_RB6) or ((rb6_scope_progression or rb7_scope_progression) and rb6r.get('status') == 'closed-owner-scope-excluded-16k-unsupported-not-qualified' and rb6r.get('evidence', {}).get('actual_16k_runtime_supported') is False),
