...
def verify(root: Path = ROOT) -> dict[str, Any]:
    ...
    checks={
      ...
      'register_rb7':(rb7.get('status')=='open-owner-support-disposition' and rb7.get('evidence',{}).get('contract_sha256')==EXPECTED_RB7) or (rb7_progression and rb7.get('status')=='closed-owner-scope-excluded-non-termux-unsupported-not-qualified' and rb7.get('evidence',{}).get('non_termux_android_context_supported') is False) or (later_progression and rb7.get('status')=='closed-appuid-non-termux-runtime-qualified-supported' and rb7.get('evidence',{}).get('app_uid_non_termux_runtime_accepted') is True),
      ...
      'nonclaims':(sb.get('non_termux_android_context_qualified') is False and sb.get('publication_authorized') is False) or (later_progression and sb.get('actual_16k_runtime_supported') is False and sb.get('publication_authorized') is False),
    }
