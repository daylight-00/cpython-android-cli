...
def verify(root:Path|None=None)->dict:
    ...
    checks={
      ...
      'catalog':e3.get('deliverable',{}).get('current_bounded_transition')=='rb1-successor-r3-explicit-owner-approval' and e3.get('activation',{}).get('accepted_authority_sha256')==EXPECTED_AUTH and e3.get('work_class')=='L',
      'register_rb7':rb7.get('status')=='closed-owner-scope-excluded-non-termux-unsupported-not-qualified' and rb7.get('evidence',{}).get('support_scope_authority_sha256')==EXPECTED_AUTH and rb7.get('evidence',{}).get('non_termux_android_context_supported') is False,
      ...
      'nonclaims':sb.get('minimum_supported_android_api_declared') is False and sb.get('publication_authorized') is False,
    }
