...
OUTPUT_DIR="${OUTPUT_DIR:-$PROJECT_ROOT/results/local/stage3c-phase5-gate3d-final-uninstall-design}"
PYTHON="${PYTHON:-python3}"
rm -rf "$OUTPUT_DIR"
mkdir -p "$OUTPUT_DIR"
cp "$SCRIPT_DIR/gate3d-final-uninstall-matrix.json" "$OUTPUT_DIR/matrix.json"
"$PYTHON" -I -B -S "$SCRIPT_DIR/verify-gate3d-final-uninstall-design.py" \
...
output.write_text(json.dumps({'schema_version':1,'index_kind':'stage3c-phase5-gate3d-final-uninstall-design-result-index','file_count':len(files),'files':files},indent=2,sort_keys=True)+'\n')
PY
printf 'STAGE3C_PHASE5_GATE3D_FINAL_UNINSTALL_DESIGN=PASS\n'
printf 'OUTPUT_DIR=%s\n' "$OUTPUT_DIR"
