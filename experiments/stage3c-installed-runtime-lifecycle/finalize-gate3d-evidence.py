...
def write_safety(root: Path, safety_output: Path, index_output: Path) -> int:
    ...
    result = {
        "schema_version": 1,
        "verification_kind": "stage3c-phase5-gate3d-result-tree-safety",
        "counts": counts,
        "unsafe_links": unsafe_links,
        "special_entries": special_entries,
        "pass": not unsafe_links and not special_entries,
        "claim_boundary": "Pre-archive filesystem safety only; final Gate 3D acceptance still requires independent archive and root-index inspection.",
    }
...
def write_index(root: Path, safety_output: Path, index_output: Path) -> int:
    ...
    result = {
        "schema_version": 1,
        "index_kind": "stage3c-phase5-gate3d-final-uninstall-result-index",
