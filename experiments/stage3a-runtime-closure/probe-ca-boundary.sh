...
# Stage 3-A CA trust non-ELF boundary probe.
...
PYTHONPYCACHEPREFIX="$RESULTS_DIR/pycache-ca-probe" \
"$PYTHON_BIN" \
    "$SCRIPT_DIR/probe-ca-boundary.py" \
    --python-bin "$PYTHON_BIN" \
    --termux-prefix "$TERMUX_PREFIX" \
    --output-dir "$RESULTS_DIR"

printf '\n'
printf 'Result: %s\n' "$RESULTS_DIR/ca-boundary-probe.json"
printf '\n'
echo "CA_BOUNDARY_PROBE=COMPLETE"
