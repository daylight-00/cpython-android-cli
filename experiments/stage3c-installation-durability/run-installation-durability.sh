...
# Stage 3-C Phase 4 Gate 4: durability primitive and ordering protocol.
...
GATE3_RESULTS="${GATE3_RESULTS:-$RESULTS_ROOT/termux/stage3c-phase4-installation-recovery}"
CANONICAL_PREFIX="${CANONICAL_PREFIX:-$WORK_ROOT/termux/stage3b-promoted-runtime/prefix}"
RESULTS_DIR="${RESULTS_DIR:-$RESULTS_ROOT/termux/stage3c-phase4-installation-durability}"
DURABILITY_WORK="${DURABILITY_WORK:-$WORK_ROOT/termux/stage3c-phase4-installation-durability}"
...
SCENARIOS="$SCRIPT_DIR/run-durability-scenarios.py"
VERIFIER="$SCRIPT_DIR/verify-durability-scenarios.py"
INPUT_DIR="$RESULTS_DIR/input/gate3"
...
for file in \
    "$GATE3_RESULTS/scenario.json" \
    "$GATE3_RESULTS/verification.json" \
    "$GATE3_RESULTS/workflow-status.json" \
    "$GATE3_RESULTS/result-index.json" \
    "$GATE3_RESULTS/input/gate2/result-index.json" \
    "$GATE3_RESULTS/input/gate2/input/contract/contract-index.json" \
    "$LOCAL_SCRIPT_RUNNER" \
    "$FINGERPRINT" \
    "$SCRIPT_DIR/durability_common.py" \
    "$SCENARIOS" \
    "$VERIFIER"; do
    [[ -f "$file" ]] || {
...
rm -rf "$RESULTS_DIR" "$DURABILITY_WORK"
mkdir -p "$INPUT_DIR"
cp -a "$GATE3_RESULTS"/. "$INPUT_DIR"/

printf 'Gate 3 results:         %s\n' "$GATE3_RESULTS"
printf 'Durability work:       %s\n' "$DURABILITY_WORK"
printf 'Results:               %s\n\n' "$RESULTS_DIR"
...
print(text, end="")
raise SystemExit(0 if passed else 50)
PY
}
...
result = {
    ...
    "pass": False,
    "blocked": True,
    "blocked_by": "durability_scenarios",
    "scenario_returncode": int(sys.argv[2]),
    "check_count": 0,
    "checks": {},
    "failed_checks": ["durability_verification_blocked"],
}
...
returncodes = {
    "durability_scenarios": int(sys.argv[2]),
...
result = {
    "schema_version": 1,
    "index_kind": "stage3c-phase4-installation-durability-result-index",
...
printf 'Durability contract: %s\n' "$RESULTS_DIR/durability-contract.json"
...
if [[ $final_rc -ne 0 ]]; then
    echo "STAGE3C_PHASE4_INSTALLATION_DURABILITY=FAIL rc=$final_rc"
    exit "$final_rc"
fi

echo "INSTALLATION_DURABILITY_ACCEPTED_INPUTS=PASS"
echo "INSTALLATION_DURABILITY_SCENARIOS=64/64 PASS"
echo "INSTALLATION_DURABILITY_VERIFICATION=53/53 PASS"
echo "INSTALLATION_DURABILITY_REGULAR_FSYNC=PASS"
echo "INSTALLATION_DURABILITY_DIRECTORY_FSYNC=PASS"
echo "INSTALLATION_DURABILITY_ATOMIC_REPLACE=PASS"
echo "INSTALLATION_DURABILITY_PARENT_FSYNC=PASS"
echo "INSTALLATION_DURABILITY_TRANSACTION_ORDER=PASS"
echo "INSTALLATION_DURABILITY_NEGATIVE_CONTROLS=PASS"
echo "INSTALLATION_DURABILITY_INPUT_MUTATION_CHECK=PASS"
echo "STAGE3C_PHASE4_INSTALLATION_DURABILITY=PASS"
