...
STATE=Path('docs/current/STATE.json');REG=Path('docs/documentation/document-registry.json');TARGETS=Path('experiments/document-legacy-authority-decoupling/navigation-targets.json')
...
def assignment(d,targets):
 ...
 if p.startswith('docs/history/legacy-authority-bindings/') and p!='docs/history/legacy-authority-bindings/README.md':return 'docs/history/legacy-authority-bindings/README.md'
...
document migration           Phase {c['completed_phase']} complete; migration closed
...
> **Generated root:** registry v5 + {link(root,'docs/current/STATE.json')}.
...
'''
 ...
 # Legacy binding compatibility snapshots.
 t='docs/history/legacy-authority-bindings/README.md';items=[d for d in by[t] if d['path']!=t];groups=defaultdict(list)
 for d in items:
  parts=d['path'].split('/');groups['/'.join(parts[3:5])].append(d)
 out[t]='# Legacy Authority Compatibility Snapshots\n\n> **Generated view:** registry v5. Do not hand-edit.\n> These files preserve exact bytes referenced by grandfathered historical authorities. Current live documents are not constrained to these digests.\n\n- [Decoupling policy](../../documentation/LEGACY_AUTHORITY_DECOUPLING.md)\n- [Resolution map](../../documentation/legacy-authority-decoupling-map.json)\n\n'+ '\n\n'.join(f"## `{'/'.join(g.split('/'))}`\n\n"+'\n'.join(entry(t,d) for d in sorted(v,key=lambda x:x['path'])) for g,v in sorted(groups.items()))+'\n'
 # Experiments.
...
> **Generated view:** registry v5 + {link(src,'docs/current/STATE.json')}.
...
def main():
 ...
 out={'pass':not bad,'mismatched_targets':bad,'current_target_count':4,'navigation_target_count':15,'target_count':19};print(json.dumps(out,indent=2,sort_keys=True));return 0 if not bad else 1
